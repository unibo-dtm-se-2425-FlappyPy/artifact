...
class testBoundaryChecking(unittest.TestCase):
    ...
    def test_ground_collision_detection(self):
        ...
        self.assertTrue(ground_collision, "Bird at ground level should trigger collision")
    ...
    def test_no_ground_collision_when_airborne(self):
        ...
        ground_collision = self.bird.check_collision_with_ground()
        self.assertFalse(ground_collision, "Airborne bird should not trigger ground collision")
    ...
    def test_no_ceiling_collision_when_below(self):
        ...
        ceiling_collision = self.bird.check_collision_with_ceiling()
        self.assertFalse(ceiling_collision, "Bird below ceiling should not trigger collision")

    def test_boundary_edge_cases(self):
        """Test collision detection at exact boundary edges"""
        # Test ground collision at exact boundary
        self.bird.y = WINDOW_HEIGHT - self.bird.height
        self.assertTrue(self.bird.check_collision_with_ground(), "Bird at exact ground boundary should collide")
        
        # Test just above ground
        self.bird.y = WINDOW_HEIGHT - self.bird.height - 1
        self.assertFalse(self.bird.check_collision_with_ground(), "Bird just above ground should not collide")
        
        # Test ceiling collision at exact boundary
        self.bird.y = 0
        self.assertTrue(self.bird.check_collision_with_ceiling(), "Bird at exact ceiling boundary should collide")
        
        # Test just below ceiling
        self.bird.y = 1
        self.assertFalse(self.bird.check_collision_with_ceiling(), "Bird just below ceiling should not collide")
